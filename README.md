...
1. 支持数据更新鉴权
2. 兼容不同schema相同数据表名的监控, 以及对于不同的schema的功能完整性测试, 需要重构对于表名的处理逻辑
...
7. 创建时间和更新时间自动更新
8. 使用更新/插入/查询鉴权可以通过继承不同的方法实现配置不同获取鉴权数据的目的，
而不是通过在注解中配置type的方式，减少配置项，同时支持默认获取鉴权数据的方法
9. 获取鉴权方法使用正则匹配代替原本的list参数，简化多数据表的鉴权的配置成本
10. 用户提示，mybatis的\<select\>标签和mapper文件中@Select标识不要写错, 在正常情况下可能没有影响,
